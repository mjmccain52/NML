...
def update_screen(target, load=None, cursor=None):
    ...
    screen.fill(WHITE)
    pygame.draw.circle(screen, RED, (50,50), 30)
    pygame.draw.circle(screen, target.color, target.position, target.radius)
    if loading:
        pygame.draw.circle(screen, load.color, load.position, load.radius)
    if cursor:
        pygame.draw.circle(screen, cursor.color, cursor.position, cursor.radius)
    pygame.display.flip()
...
ct = Circle(locations[0], BLACK, TARGET_SIZE)
update_screen(ct)
