...
class ClockLeapListener(Leap.Listener):
    ...
    def on_frame(self, controller):
        ...
        if not frame.fingers.is_empty:
            ...
            # This next part depends on the Leap "guessing" where your monitor
            # is, since Leap removed the Calibration tool. Waiting for an
            # update in the API.
            monitor = controller.located_screens[0]
            normal = monitor.intersect(fingers[0],True) # Values from 0 to 1
                                                        # showing relative
                                                        # intersection point
                                                        # on monitor
            
            # Verify that user is pointing somewhere on the screen
            if normal and not (isnan(normal[0]) or isnan(normal[1])):
                
                # Multiply normalized values by screen resolution to get pixel
                # locations
                xPoint = screen_x * normal[0]
                yPoint = screen_y * normal[1]
                zFactor = fingers[0].tip_position[2] / -20.0
                self.move_mouse(int(xPoint), int(screen_y - yPoint), zFactor)
            else:
                cursor = None
